...
public class UpdateDownloadDialog extends DialogFragment {
    ...
    public interface DownloadActionListener { void onCancelled(); }
    public void setActionListener(DownloadActionListener l) { this.listener = l; }
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        btnCancel.setOnClickListener(v -> { if (listener != null) listener.onCancelled(); dismiss(); });
    ...
    public void updateProgress(int progress) {
        if (getDialog() != null && progressBar != null && tvProgress != null) {
            requireActivity().runOnUiThread(() -> {
                progressBar.setProgress(progress); tvProgress.setText(progress + "%");
                if (progress >= 100) btnCancel.setText("安装");
            });
        }
    }
    ...
    public void showSuccess() {
        if (getDialog() != null) {
            requireActivity().runOnUiThread(() -> {
                tvProgress.setText("下载完成"); progressBar.setProgress(100); btnCancel.setText("安装");
            });
        }
    }
}
