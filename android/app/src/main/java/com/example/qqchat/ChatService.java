...
public class ChatService extends Service {
    ...
    @Override public void onCreate() { super.onCreate(); instance = this; nativeInit(); }
    @Override public IBinder onBind(Intent intent) { return binder; }
    @Override public void onDestroy() { nativeDestroy(); instance = null; super.onDestroy(); }

    public static ChatService getInstance() { return instance; }
    public void setListener(ServiceListener listener) { this.listener = listener; }
    public void connect() { nativeConnect(SERVER_HOST, SERVER_PORT); }
    public void disconnect() { nativeDisconnect(); }
    public boolean isConnected() { return nativeIsConnected(); }
    public void login(String username, String password) { nativeLogin(username, password); }
    public void sendMessage(int toId, String content) { nativeSendMessage(toId, content); }
    public void getFriendList() { nativeGetFriendList(); }
    public void addFriend(int friendId) { nativeAddFriend(friendId); }
    public void register(String username, String password, String sq, String sa) { nativeRegister(username, password, sq, sa); }
    public void getSecurityQuestion(String username) { nativeGetSecurityQuestion(username); }
    public void resetPassword(String username, String np, String sa) { nativeResetPassword(username, np, sa); }
    public void checkVersion(int clientVersion) { nativeCheckVersion(clientVersion); }
    ...
    private void onLoginResult(int errorCode, int userId, String username) {
        if (errorCode == 0) { this.currentUserId = userId; this.currentUsername = username; }
        if (listener != null) listener.onLoginResult(errorCode, userId, username);
    }
    private void onMessageReceived(int fromId, String content, long timestamp) {
        if (listener != null) listener.onMessageReceived(fromId, content, timestamp);
    }
    private void onFriendListReceived(int errorCode, int[] friendIds, String[] friendNames) {
        if (listener != null) {
            java.util.ArrayList<Integer> ids = new java.util.ArrayList<>();
            java.util.ArrayList<String> names = new java.util.ArrayList<>();
            for (int i = 0; i < friendIds.length; i++) { ids.add(friendIds[i]); names.add(friendNames[i]); }
            listener.onFriendListReceived(errorCode, ids, names);
        }
    }
    private void onRegisterResult(int errorCode, int userId) {
        if (listener != null) listener.onRegisterResult(errorCode, userId);
    }
    private void onSecurityQuestionResult(int errorCode, String question) {
        if (listener != null) listener.onSecurityQuestionResult(errorCode, question);
    }
    private void onResetPasswordResult(int errorCode) {
        if (listener != null) listener.onResetPasswordResult(errorCode);
    }
    private void onVersionCheckResult(int errorCode, int sv, String url, String desc) {
        if (listener != null) listener.onVersionCheckResult(errorCode, sv, url, desc);
    }

    static { System.loadLibrary("qqchat_core"); }
